...
class MortgageDetails extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final familyRelation = ref.read(familyRelationListProvider).firstWhere(
        (familyRelation) => familyRelation.id == mortgage.familyRelationId);
    final mortgageMaterial = ref.read(mortgageMaterialListProvider).firstWhere(
        (mortgageMaterial) =>
      ...
      body: SingleChildScrollView(
        ...
        child: SingleChildScrollView(
          ...
          child: DataTable(
            ...
            rows: [
              buildDataRow(context, 'Depositor Name', mortgage.depositorName),
              buildDataRow(context, 'Relative Name', mortgage.relativeName),
              ...
              buildDataRow(
                  context, 'Interest Rate', mortgage.interestRate.toString()),
              buildDataRow(
                  context,
                  'Interest Type',
              ...
              buildDataRow(
                  context, 'Additional Details', mortgage.additionalDetails),
              buildDataRow(context, 'Item', item.name),
              buildDataRow(context, 'Family Relation', familyRelation.name),
              buildDataRow(context, 'Mortgage Material', mortgageMaterial.name),
            ],
          ),
        ),
      ),
